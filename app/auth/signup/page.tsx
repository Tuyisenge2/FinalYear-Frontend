...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupFormData) => {
   ...
   return (
     <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 transition-colors duration-300 py-12">
       <motion.div
        ...
        <div className="backdrop-blur-xl bg-blue-50/80 dark:bg-gray-900/80 border border-blue-200 dark:border-gray-800 rounded-3xl shadow-lg overflow-hidden transition-colors duration-300">
          <div className="p-8 sm:p-10">
            ...
            <div className="text-center mb-8">
               <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100 mb-2">Create Account</h2>
               <p className="text-sm text-gray-600 dark:text-gray-400">Join the Irondo Security System</p>
             </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input id="fullName" type="text" placeholder="Jean Kanimba"
                      className="w-full bg-white dark:bg-gray-950 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 h-12 rounded-xl pl-11 focus:ring-emerald-500/50 placeholder:text-gray-400 dark:placeholder:text-gray-500 transition-colors duration-300" {...register("fullName")} />
                  </div>
                  {errors.fullName && <p className="text-xs text-red-600 dark:text-red-400 ml-1">{errors.fullName.message}</p>}
                </div>
                 ...
                 <div className="relative">
                   <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    ...
                    <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} autoComplete="new-password" placeholder="••••••••"
                      className="w-full bg-white dark:bg-gray-950 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-gray-100 h-12 rounded-xl pl-11 pr-11 focus:ring-emerald-500/50 placeholder:text-gray-400 dark:placeholder:text-gray-500 transition-colors duration-300" {...register("confirmPassword")} />
                   <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)}
                     className="absolute right-0 inset-y-0 pr-4 flex items-center text-gray-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
                     {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                   </button>
                 </div>
                 {errors.confirmPassword && <p className="text-xs text-red-600 dark:text-red-400 ml-1">{errors.confirmPassword.message}</p>}
              </div>
              ...
              <Button type="submit" className="w-full h-12 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-base mt-4 transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_25px_rgba(16,185,129,0.5)]" disabled={loading}>
                 {loading ? (
                   ...
                   <span className="flex items-center justify-center gap-2">Sign Up <ChevronRight className="h-4 w-4" /></span>
                 )}
              </Button>
            </form>
          </div>
           ...
           <div className="bg-blue-50/50 dark:bg-gray-900/50 border-t border-blue-200 dark:border-gray-800 p-6 text-center transition-colors duration-300">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Already have an account? <a href="/auth/login" className="text-emerald-600 dark:text-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-400 font-medium ml-1 transition-colors">Log in</a>
              </p>
            </div>
        </div>
      </motion.div>
